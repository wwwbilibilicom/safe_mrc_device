...
Safeguarder::Safeguarder(const std::string& port)
    : port_(port) {
    rs485_serial_ = std::make_unique<RS485Serial>(port_);
    master_rs485_device_collection_ = std::make_unique<RS485DeviceColllection>(*rs485_serial_);
    mrc_component_ = std::make_unique<MRCComponent>(*rs485_serial_);
}

void Safeguarder::init_mrcs(const std::vector<MRCType>& mrc_types, const std::vector<uint8_t>& rs485_ids) {
    mrc_component_->init_mrc_devices(mrc_types, rs485_ids);
    register_mrc_device_collection(*mrc_component_);
}

void Safeguarder::register_mrc_device_collection(SafeMRCDeviceCollection& device_collection) {
