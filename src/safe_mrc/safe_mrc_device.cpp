...
SafeMRCRS485Device::SafeMRCRS485Device(MRC& mrc)
    : RS485Device(mrc.get_rs485_id()), mrc_(mrc), mrc_mode_(MRCMode::FREE) {}
MRCCmdFrame SafeMRCRS485Device::create_cmd_frame(MRCMode mode,
                                                 float current_A) {
  MRCCmdFrame frame;
  frame.head[0] = 0xFE;
  frame.head[1] = 0xEE;
  ...
  frame.CRC16Data =
      crc_ccitt(0xFFFF, reinterpret_cast<uint8_t*>(&frame), sizeof(frame) - 2);
...
void SafeMRCRS485Device::parse_fdk_frame(MRCFdkFrame& frame) {
  ...
  if (frame.id != rs485_id_) {
    std::cerr << "Warning: MRC ID mismatch! Expected "
              << static_cast<int>(rs485_id_) << ", got "
              << static_cast<int>(frame.id) << std::endl;
    return;
  }
  uint16_t crc_calculated =
      crc_ccitt(0xFFFF, reinterpret_cast<uint8_t*>(&frame), sizeof(frame) - 2);
  if (crc_calculated != frame.CRC16Data) {
    std::cerr << "Warning: CRC mismatch! Calculated " << std::hex
              << crc_calculated << ", received " << std::hex << frame.CRC16Data
              << std::dec << std::endl;
    return;
  }
  ...
  mrc_.update_state(result.position, result.velocity, result.current,
                    result.collision, result.mode);
}

void SafeMRCRS485Device::callback(MRCFdkFrame& frame) {
  this->parse_fdk_frame(frame);
}
