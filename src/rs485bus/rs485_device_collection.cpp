...
bool RS485DeviceColllection::enable_rx_thread() {
    ...
    try {
        ...
        rx_thread_ = std::thread(&RS485DeviceColllection::rxThread, this);
        return true;
    }
...
void RS485DeviceColllection::disable_rx_thread() {
...
void RS485DeviceColllection::add_device(const std::shared_ptr<RS485Device>& device) {
    if (!device) return;
...
void RS485DeviceColllection::remove_device(const std::shared_ptr<RS485Device>& device) {
    if (!device) return;
...
void RS485DeviceColllection::dispatch_frame_callback(safe_mrc::MRCFdkFrame & frame){
...
void RS485DeviceColllection::rxThread() {
    ...
    while(rx_thread_running_){
        try{
            ...
            if(n>0){
                rs485_serial_.rx_buffer_.insert(rs485_serial_.rx_buffer_.end(), temp.begin(), temp.begin()+n);
...
bool RS485DeviceColllection::unpackStream(std::vector<uint8_t>& buf){
    size_t idx=0, L=buf.size();
    while(idx + sizeof(MRCFdkFrame) <= L){
        if(buf[idx]!=0xFE || buf[idx+1]!=0xEE){ idx++; continue; }
        ...
        if(crc_recv != crc_calc){ idx++; continue; } // 同步
